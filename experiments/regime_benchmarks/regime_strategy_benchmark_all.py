#
# ============================================================
# REGIME STRATEGY BENCHMARK (_all): HMM, HMM2, HMM3, GMM, JM, JM2, CJM + BH
# ============================================================
#
# Laajennettu versio regime_strategy_benchmark_soft3.py:sta. Sama soft-weight
# strategia (w = 1 - P(Bear), 1 kk viiveella), mutta vertailussa ovat KAIKKI
# regiimimallit:
#   - HMM   : Gaussian HMM (src/HMM.py)
#   - HMM2  : HMM-parannukset (src/HMM2.py)
#   - HMM3  : sticky transmat + realized vol + EWMA smoothing (src/HMM3.py)
#   - GMM   : Gaussian mixture (src/GMM.py)
#   - JM    : hard statistical jump model (src/JM.py)
#   - JM2   : naiivi continuous-yritys, plain L1 (src/JM2.py)
#   - CJM   : continuous statistical jump model, squared L1 (src/CJM.py)
#   - BH    : buy-and-hold (aina taysin investoituna)
#
# Output-tiedostot "_all"-suffiksilla data/01_raw/outputs/ -kansiossa.
#
# HUOM: ROOT osoittaa repo-juureen (parents[1]), koska tama skripti elaa
# regime_methods_comparison/ -alikansiossa. (soft/soft2/soft3-versioissa on
# Path(__file__).parent joka osoittaa nyt vaaraan paikkaan.)
#
# Kaytto: aja ensin src/HMM.py, src/HMM2.py, src/HMM3.py, src/GMM.py,
# src/JM.py, src/JM2.py ja src/CJM.py.
#
import os
import warnings
...
HMM3_PATH = OUTPUT_DIR / "hmm3_regimes_monthly_no_lookahead_ml.csv"
...
RETURNS_OUTPUT = OUTPUT_DIR / "regime_strategy_returns_all.csv"
METRICS_OUTPUT = OUTPUT_DIR / "regime_strategy_metrics_all.csv"
REGIME_ACCURACY_OUTPUT = OUTPUT_DIR / "regime_strategy_regime_accuracy_all.csv"
CUMULATIVE_PLOT = OUTPUT_DIR / "cumulative_returns_all.png"
DRAWDOWN_PLOT = OUTPUT_DIR / "drawdowns_all.png"
WEIGHT_PLOT = OUTPUT_DIR / "regime_weights_all.png"
...
# (model name, ml-safe file, regime column, continuous bear-score column)
MODEL_SPECS = [
    ("HMM", HMM_PATH, "HMM_Regime", "Bear_Prob_MeanWindow"),
    ("HMM2", HMM2_PATH, "HMM_Regime", "Bear_Prob_MeanWindow"),
    ("HMM3", HMM3_PATH, "HMM_Regime", "Bear_Prob_MeanWindow"),
    ("GMM", GMM_PATH, "GMM_Regime", "Bear_Prob_MeanWindow"),
    ("JM", JM_PATH, "JM_Regime", "Bear_StateMeanWindow"),
    ("JM2", JM2_PATH, "JM2_Regime", "Bear_Prob_MeanWindow"),
    ("CJM", CJM_PATH, "CJM_Regime", "Bear_Prob_MeanWindow"),
]
MODEL_COLORS = {
    "BH": "black",
    "HMM": "tab:blue",
...
def plot_cumulative_returns(strategies: pd.DataFrame, model_names: list, output_path: Path):
    ...
    ax.set_xlabel("Date")
    ax.set_title(
        f"All regime strategies (HMM/HMM2/HMM3/GMM/JM/JM2/CJM) vs. BH "
        f"({STOXX_TICKER}, TC={TRANSACTION_COST*1e4:.0f} bps)"
    )
...
def plot_drawdowns(strategies: pd.DataFrame, model_names: list, output_path: Path):
    ...
    ax.set_xlabel("Date")
    ax.set_title("Drawdown comparison (all models, soft weights, no transaction costs)")
...
def print_pairwise(metrics_df: pd.DataFrame, model_a: str, model_b: str):
    df = metrics_df.set_index("Model")
    if model_a not in df.index or model_b not in df.index:
        return
    a = df.loc[model_a]
    b = df.loc[model_b]
    delta_sharpe = float(b["Sharpe"]) - float(a["Sharpe"])
    delta_mdd = float(b["MaxDrawdown"]) - float(a["MaxDrawdown"])
    delta_to = float(b["AnnualizedTurnover"]) - float(a["AnnualizedTurnover"])
    a_lo, a_hi = float(a["SharpeCI_Lower"]), float(a["SharpeCI_Upper"])
    b_lo, b_hi = float(b["SharpeCI_Lower"]), float(b["SharpeCI_Upper"])
    ci_overlap = not (b_hi < a_lo or b_lo > a_hi)
    print(f"\n=== {model_b} vs {model_a} ===")
    ...
    print(f"  ΔTurnover : {delta_to:+.2f}/yr  ({'lower' if delta_to < 0 else 'higher'})")
    print(
        f"  Sharpe CIs overlap: {ci_overlap} "
...
def print_comparison(
    ...
    print(f"\n=== Metrics (TC={TRANSACTION_COST*1e4:.1f} bps, soft weights, _all run) ===")
    ...
    # HMM family progression
    print_pairwise(metrics_df, "HMM", "HMM2")
    print_pairwise(metrics_df, "HMM2", "HMM3")
    print_pairwise(metrics_df, "HMM", "HMM3")
    # Jump model progression: hard JM -> naive continuous JM2 -> paper-faithful CJM
    print_pairwise(metrics_df, "JM", "JM2")
    print_pairwise(metrics_df, "JM2", "CJM")
    print_pairwise(metrics_df, "JM", "CJM")
...
def main():
    ...
    print(
        f"_all regime benchmark (HMM, HMM2, HMM3, GMM, JM, JM2, CJM): "
    ...
    if not model_dfs:
        raise RuntimeError(
            "No regime files found. Run HMM.py, HMM2.py, HMM3.py, GMM.py, "
            "JM.py, JM2.py and CJM.py first."
        )
