#
# ============================================================
# REGIME STRATEGY BENCHMARK v2: SOFT WEIGHTS, NO TC
# ============================================================
#
# Toinen, objektiivisempi versio Shu et al. -tyylisesta sanity checkista.
# Eroavaisuudet versioon regime_strategy_benchmark.py:
#
# 1) Transaktiokustannukset oletuksena 0.
#    v1:ssa 10 bps/one-way. TC-vaikutuksen voi mitata erikseen jos haluaa, mutta
#    sanity check siita oliko regiimi-informaatio talouden kannalta hyodyllista
#    on selkeampi ilman TC-haittaa.
#
# 2) Soft-painot binarisen 0/1:n sijaan.
#    v1:n 0/1-strategia putoaa cashiin koko kuukaudeksi heti kun malli sanoo
#    "Bear". Tama vahvistaa lag-ongelmaa: regiimi tunnistetaan kun pohjat on jo
#    tehty, ja palautus tulee usein nopeasti.
#    Tassa kaytetaan jatkuvaa painoa w = 1 - P(Bear):
#       HMM, GMM: w = 1 - Bear_Prob_MeanWindow  (smoothed posteriori)
#       JM:      w = 1 - Bear_StateMeanWindow   (hard-state window freq, ei
#                                               posteriori)
#    Soft-paino antaa osittaista exposurea kun malli on epavarma → ei putoa
#    kokonaan cashiin yksittaisesta noisya signaalista.
#
# 3) Bootstrap-luottamusvalit Sharpelle.
#    16 vuotta = ~190 kuukautta. Sharpe-erot 0.1–0.2 mahtuvat helposti otoksen
#    satunnaisvaihteluun. i.i.d.-bootstrap (oletus 1000 resamplea) tarjoaa 95 %
#    CI:n jokaiselle strategialle, jotta nakee onko ero BH:hon tilastollisesti
#    olemassa.
#
# 4) Regiimi-tarkkuus -taulu.
#    Lasketaan kunkin mallin diskreettien Bull/Bear/Transition-kuukausien
#    keskimaarainen STOXX-tuotto. Jos Bear-kuukausien tuotto on selvasti
#    negatiivinen ja Bull-kuukausien positiivinen, regiimisignaali sisaltaa
#    informaatiota; jos kaikki keskiarvot ovat samanlaisia, regiimi on melu.
#
# 5) Soft-painot tarkoittavat ettei kaytossa enaa ole Switches-mittaria yhta
#    informatiivisena (paino voi muuttua tasaisesti). Tilalle vaihdetaan
#    keskimaarainen painon |Δw| (jatkuva turnover-proxy).
#
import os
import warnings
...
RETURNS_OUTPUT = OUTPUT_DIR / "regime_strategy_returns_soft.csv"
METRICS_OUTPUT = OUTPUT_DIR / "regime_strategy_metrics_soft.csv"
REGIME_ACCURACY_OUTPUT = OUTPUT_DIR / "regime_strategy_regime_accuracy_soft.csv"
CUMULATIVE_PLOT = OUTPUT_DIR / "cumulative_returns_hmm_gmm_jm_soft.png"
DRAWDOWN_PLOT = OUTPUT_DIR / "drawdowns_hmm_gmm_jm_soft.png"
WEIGHT_PLOT = OUTPUT_DIR / "regime_weights_hmm_gmm_jm_soft.png"

START_DATE = os.getenv("BENCHMARK_START", "2010-01-01")
END_DATE = os.getenv("BENCHMARK_END", "2026-04-30")
TRANSACTION_COST = float(os.getenv("TRANSACTION_COST", "0.0"))  # v2 default: TC off
...
def load_regime_signal(
    ...
    # Soft weight: 1 - Bear score (posterior for HMM/GMM, hard-state freq for JM).
    # Clip defensively in case of numerical leaks outside [0, 1].
...
def compute_strategy_returns(
    ...
    for name in model_names:
        ...
        out[f"{name}_Return"] = net  # TC=0 by default → gross == net
...
def compute_metrics(
    ...
    # Continuous turnover proxy: average monthly |Δw|, annualized
...
def compute_regime_accuracy(strategies: pd.DataFrame, model_names: list) -> pd.DataFrame:
    """Mean STOXX return conditional on each model's discrete regime call.

    Sanity check: is Bear actually associated with negative monthly returns?
    """
    rows = []
...
def plot_cumulative_returns(strategies: pd.DataFrame, model_names: list, output_path: Path):
    ...
    ax.set_xlabel("Date")
    ax.set_title(
        f"Soft-weight regime strategies vs. Buy-and-Hold ({STOXX_TICKER}, TC={TRANSACTION_COST*1e4:.0f} bps)"
    )
...
def plot_drawdowns(strategies: pd.DataFrame, model_names: list, output_path: Path):
    ...
    for name in model_names:
        ...
        ax.plot(dd.index, dd.values, color=MODEL_COLORS[name], linewidth=1.4, label=f"{name} soft")
...
def plot_regime_weights(strategies: pd.DataFrame, model_names: list, output_path: Path):
    ...
    for ax, name in zip(axes, model_names):
        ...
        ax.fill_between(weight.index, 0, weight.values, color=MODEL_COLORS[name], alpha=0.35)
        ax.plot(weight.index, weight.values, color=MODEL_COLORS[name], linewidth=1.1)
...
def print_comparison(
    ...
    print(f"\n=== Metrics (TC={TRANSACTION_COST*1e4:.1f} bps, soft weights) ===")
    print(metrics_df.set_index("Model").round(4).to_string())

    print("\n=== Sharpe with bootstrap 95% CI ===")
    for _, row in metrics_df.iterrows():
        print(
            f"  {row['Model']:>4}: point={row['Sharpe']:.3f}  "
    ...
    if "BH" in metrics_df["Model"].values:
        ...
        print("Sharpe vs BH (CI-based, no significance if CIs overlap):")
        for name in model_names:
            ...
            print(f"  {name:>4}: {s:.3f}  CI=[{lo:.3f}, {hi:.3f}]  → {verdict}")
    ...
    print(
        "\n  Tulkinta: jos Bull-keskituotto >> Bear-keskituotto ja Bear-kuukausista\n"
        "  enemmisto on negatiivisia, regiimisignaali sisaltaa informaatiota.\n"
        "  Jos kaikki keskiarvot ovat samanlaisia, signaali on kohinaa."
    )
...
def main():
    ...
    print(
        f"Soft-weight regime benchmark: {START_DATE} → {END_DATE}, "
    ...
    if not model_dfs:
        raise RuntimeError(
            "No regime files found. Run HMM.py, GMM.py and JM.py first to produce "
            "*_regimes_monthly_no_lookahead_ml.csv."
        )
    ...
    rf_used_default = (rf_ret.eq(0.0).all()) or (
        (rf_ret == ((1.0 + RF_FALLBACK_ANNUAL) ** (1.0 / 12.0) - 1.0)).all()
    )
    if rf_used_default:
        print("NOTE: RF series is constant — fallback in use, real cash returns are not modelled.")
