...
Samples RICs from STOXX membership intervals, fetches data with warmup,
computes the current feature set, builds a monthly panel, and adds three
raw point-in-time flags. Downstream analyses combine the flags as needed.

Raw flags written to the master:
    IsIndexMemberAtT     RIC was a STOXX 600 member at month-end
    HasCoreHistory       core rolling features (MOM_12M, Beta_252d, -IdioVol) all non-NaN
    HasNextMonthReturn   t+1 excess return is available for this row

Run:
    python -B PIT_universe.py
"""
...
DEFAULT_MASTER_OUTPUT = PROJECT_ROOT / "data" / "02_preprocessed" / "PIT_MASTER_DF_1.csv"
DEFAULT_SUMMARY_OUTPUT = PROJECT_ROOT / "data" / "diagnostics" / "PIT_universe_eligibility_by_month.csv"
DEFAULT_SAMPLE_OUTPUT = PROJECT_ROOT / "data" / "diagnostics" / "PIT_universe_sample.csv"

WARMUP_CALENDAR_DAYS = 750
DEFAULT_END_DATE = "2013-01-31"
CORE_HISTORY_FEATURES = ["MOM_12M", "Beta_252d", "-IdioVol"]
RAW_FLAG_COLUMNS = ["IsIndexMemberAtT", "HasCoreHistory", "HasNextMonthReturn"]
...
def parse_args():
    ...
    parser.add_argument(
        "--rics",
        default=None,
        help="Comma-separated RIC list. Overrides --sample-size and --seed when set.",
    )
    parser.add_argument(
        "--rics-file",
        default=None,
        help="Path to a CSV/TXT with one RIC per line (or a 'RIC' column). Overrides sampling.",
    )
    parser.add_argument(
        "--tag",
        default=None,
        help="Output filename suffix, e.g. 'retry17' -> PIT_MASTER_DF_1_retry17.csv.",
    )
...
def resolve_output_paths(args):
    def with_tag(default_path):
        if args.tag is None:
            return default_path
        p = Path(default_path)
        return p.with_name(f"{p.stem}_{args.tag}{p.suffix}")

    return (
        Path(args.output) if args.output else with_tag(DEFAULT_MASTER_OUTPUT),
        Path(args.summary_output) if args.summary_output else with_tag(DEFAULT_SUMMARY_OUTPUT),
        Path(args.sample_output) if args.sample_output else with_tag(DEFAULT_SAMPLE_OUTPUT),
    )


def load_custom_rics(args):
    """Return a sorted RIC list from --rics or --rics-file, or None if neither given."""
    if args.rics:
        rics = [r.strip() for r in args.rics.split(",") if r.strip()]
        return sorted(set(rics)) or None
    if args.rics_file:
        path = _resolve_path(args.rics_file)
        if not path.exists():
            raise FileNotFoundError(f"RIC list file not found: {path}")
        df = pd.read_csv(path) if path.suffix.lower() == ".csv" else pd.read_csv(path, header=None, names=["RIC"])
        col = "RIC" if "RIC" in df.columns else df.columns[0]
        rics = [str(r).strip() for r in df[col].dropna().tolist() if str(r).strip()]
        return sorted(set(rics)) or None
    return None
...
def load_overlapping_intervals(path, display_start, end):
    ...
    if intervals.empty:
        raise ValueError("No membership intervals overlap the requested window.")
    return intervals
...
def add_pit_flags(master, intervals):
    """Add the three raw PIT flags. No derived eligibility columns."""
    out = master.copy()
...
def eligibility_summary(master):
    """Per-month counts of each raw flag plus the AND combinations
    used downstream (ML pipeline, EW benchmark)."""
    ...
    ml_mask = (
        master["IsIndexMemberAtT"]
        & master["HasCoreHistory"]
        & master["HasNextMonthReturn"]
    )
    bench_mask = master["IsIndexMemberAtT"] & master["HasNextMonthReturn"]
    summary["MLBacktestUniverse"] = ml_mask.groupby(master["Date"]).sum()
    summary["EWBenchmarkUniverse"] = bench_mask.groupby(master["Date"]).sum()
    return summary.reset_index()


def print_flag_totals(master):
    print("\nRaw flag totals")
    for col in RAW_FLAG_COLUMNS:
    ...
    bench_mask = master["IsIndexMemberAtT"] & master["HasNextMonthReturn"]
    print("\nDownstream AND combinations")
    print(f"  ML backtest universe     {int(ml_mask.sum()):>8} / {len(master):<8}")
    print(f"  EW benchmark universe    {int(bench_mask.sum()):>8} / {len(master):<8}")


def main():
    args = parse_args()
    fetch_start = args.fetch_start or default_fetch_start(args.display_start)
    output_path, summary_output, sample_output = resolve_output_paths(args)
    output_path = _resolve_path(output_path)
    summary_output = _resolve_path(summary_output)
    sample_output = _resolve_path(sample_output)
    ...
    custom_rics = load_custom_rics(args)
    if custom_rics:
        unknown = [r for r in custom_rics if r not in set(intervals["RIC"].unique())]
        if unknown:
            print(f"WARNING: {len(unknown)} RIC(s) not found in membership intervals (will fetch but flagged as non-member):")
            for r in unknown:
                print(f"    {r}")
        universe = custom_rics
        sample_intervals = intervals.loc[intervals["RIC"].isin(universe)].copy()
    else:
    ...
    print("Point-in-time universe builder")
    ...
    print("Building monthly PIT master with the same post-processing as MASTER_DF_1...")
    master = main_pipeline.build_master_dataframe(df_features, df_idx)
    master = drop_extra_valuation_columns(master)
    master = add_pit_flags(master, sample_intervals)
